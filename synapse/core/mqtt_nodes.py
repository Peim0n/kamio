...
class BaseNode:
    ...
    async def start(self):
        ...
        # Subscribe to both current and legacy formats
        self.mqtt.subscribe(f"{topics.BASE}/{self.device_id}/#")
        self.mqtt.subscribe(f"{topics.PREFIX}/{self.device_id}/#")
        self.mqtt.subscribe(f"{topics.BASE}/{BROADCAST_ID}/#")
    ...
    def _on_mqtt_message_callback(self, client, userdata, msg):
        ...
        # Filter messages by topic before processing
        dev_id, _ = topics.parse(msg.topic)
    ...
    async def publish_raw(self, topic: str, payload: bytes, qos: int = 1, retain: bool = False):
        ...
        try:
            ...
            logger.error(f"Raw publish error: {e}")
