...
class DeviceRegistryMixin:
    ...
    def devices(self: Any) -> Dict[str, Device]:
        ...
        result: Dict[str, Device] = self.registry.instances
        return result
    ...
    def register(self: Any, device_class: Type[Device]) -> None:
        ...
        if hasattr(device_class, "Kamio_RULES") and device_class.Kamio_RULES:
            for rule_name, rule_func in device_class.Kamio_RULES.items():
                ...
                self.add_rule(
                    rule_func, device=device_class, fields=fields, description=description
                )
    ...
    async def create_device(self: Any, device_id: str, device_type: str, **kwargs) -> Device:
        ...
        Args:
            ...
            **kwargs: Passed to the device constructor and ``on_init``.

        Returns:
            The created :class:`Device` instance.
        """
        cls = self.registry.get_class(device_type)
        if not cls:
            raise ValueError(f"Device type '{device_type}' not registered")

        # Separate constructor kwargs from on_init kwargs so on_init does not
        # receive unexpected parameters like 'driver' or 'keepalive_interval'.
        init_kwargs = {k: v for k, v in kwargs.items() if k not in ("driver", "keepalive_interval")}
        device_instance = cls(**kwargs)
        device_instance.app = self
        await device_instance.on_init(**init_kwargs)
        ...
        # Register instance and node before starting so dispatch works.
        self.registry.register_instance(device_id, device_instance)
        with self._device_nodes_lock:
            self._device_nodes[device_id] = node

        # Start the node; roll back registration on failure.
        if self._is_running:
            try:
                await node.start()
            except Exception as e:
                logger.error(f"Failed to start node for '{device_id}': {e}", exc_info=True)
                with self._device_nodes_lock:
                    self._device_nodes.pop(device_id, None)
                self.registry.unregister_instance(device_id)
                raise
        ...
        result: Device = device_instance
        return result

    async def remove_device(self: Any, device_id: str) -> None:
        """Stop and remove a device instance.

        Also removes device-level rules registered for this device's class
        and clears HA discovery entries if HA discovery is enabled.
        """
        ...
        # Clear HA discovery entries so HA doesn't show the device as unavailable.
        if getattr(self, "_ha_discovery_enabled", False) and self.ha_discovery is not None:
            try:
                await self.ha_discovery.clear(device_instance)
            except Exception as e:
                logger.warning(f"Failed to clear HA discovery for '{device_id}': {e}")
    ...
    async def add_device(self: Any, device_id: str, device_class: Type[Device], **kwargs) -> Device:
        ...
        Auto-registers the class if it is not already registered.

        Args:
            device_id: Unique identifier for the device instance.
            device_class: A :class:`Device` subclass to instantiate.
            **kwargs: Passed to the device constructor and ``on_init``.
        ...
        if self.registry.get_instance(device_id) is not None:
            raise ValueError(
        ...
        result: Device = await self.create_device(device_id, device_type, **kwargs)
        return result
